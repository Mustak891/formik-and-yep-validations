...
export default function Updatedata(){
    ...
    const formvalidationSchema = yup.object({
      ...
      const { handleSubmit, values, handleChange, handleBlur, errors, touched } = useFormik({
        initialValues:{studentname: localStorage.getItem('studentname') , book: localStorage.getItem('book'),  number: localStorage.getItem('number'), email: localStorage.getItem('email'), password: localStorage.getItem('password'), id: localStorage.getItem('id')},
 ...
 <div className='image'>
   <img style={{marginLeft:"100px", height:"400px", width:"400px", objectFit:"cover"}} src="https://miro.medium.com/max/512/1*lxnwe1wohMGnGA-qItmpXw.png" alt='adduser ' />
   </div>
   </div>
    </div> 
   )
